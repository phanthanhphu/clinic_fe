...
export default function ViewUserDialog({ open, onClose, user }) {
  ...
  const fallbackAvatar = useMemo(() => `${API_BASE_URL}/Uploads/users/default-user.png`, []);
  ...
  const statusMeta = useMemo(() => {
    ...
    return enabled ? { label: 'Enabled', tone: 'success' } : { label: 'Disabled', tone: 'default' };
  }, [user?.isEnabled]);
  ...
  return (
    <Dialog open={open} onClose={onClose} fullScreen={fullScreen} maxWidth="sm" fullWidth PaperProps={{ sx: paperSx }}>
      {/* Header */}
      <DialogTitle
        sx={{
          position: 'relative',
      ...
      >
        <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
          <Box>
            ...
            <Typography sx={{ opacity: 0.9, mt: 0.4, fontSize: 13 }}>Overview details and status</Typography>
          </Box>
          ...
          <Box sx={{ ...subtleCardSx, p: 2 }}>
            <Stack spacing={1.6} alignItems="center">
              ...
              <Box sx={{ textAlign: 'center' }}>
                <Typography sx={{ fontWeight: 900, fontSize: 18 }}>{user?.username || '—'}</Typography>
                <Typography sx={{ color: 'text.secondary', fontSize: 13, mt: 0.2 }}>{user?.email || '—'}</Typography>
              </Box>
              ...
              <Typography sx={{ color: 'text.secondary', fontSize: 12 }}>Avatar updates in the Edit dialog</Typography>
            </Stack>
          </Box>
          ...
          <Box sx={{ ...subtleCardSx, p: 2 }}>
            <Stack spacing={1.6}>
              ...
              <Stack spacing={1.3}>
                ...
                <InfoRow icon="ShieldTick" label="Account Status" value={statusMeta.label} />
              </Stack>
            </Stack>
          </Box>
        </Box>
